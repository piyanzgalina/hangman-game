...
const createPlaceholdersHTML = () => {
  ...
  const placeholdersHTML = arrWord.reduce(
...
const stopGame = (status) => {
  ...
  document.getElementById(
    "game"
  ).innerHTML += `<p>The word was: <span class="result-word">${word}</span></p><button id="play-again" class="button-primary px-5 py-2 mt-3">Play again</button>`;
...
export const startGame = () => {
  ...
  const image = createHangmanImg();

  gameDiv.prepend(image);

  gameDiv.insertAdjacentHTML('beforeend', '<button id="quit" class="button-secondary px-2 p-1 mt-5">Quit</button>');
